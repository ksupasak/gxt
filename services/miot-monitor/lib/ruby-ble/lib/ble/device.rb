module BLE
  ...
  class Device
    ...
    def subscribe_indicate(service, characteristic, raw: false, &block)
      _require_connection!
      uuid  = _uuid_characteristic(characteristic)
      chars = _characteristics(service)
      raise Service::NotFound,        service        if chars.nil?
      ...
      if char.flag? 'indicate'
        char.notify!
        char.on_change(raw: raw, &block)
      else
         raise OperationNotSupportedError.new("No notifications s available for characteristic #{characteristic}")
      end
    end
    ...
    @o_dev[I_PROPERTIES]
      .on_signal('PropertiesChanged') do |intf, props|
        #puts "yyyy #{@dev} #{props.inspect}"
        	
	block.call(intf, props)
