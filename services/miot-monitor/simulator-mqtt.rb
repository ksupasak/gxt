...
BROKER_HOST = '880b22a0429744488df2940b1696bfe7.s1.eu.hivemq.cloud'
...
# Fake GPS location generator
def generate_fake_gps
  {
    latitude:  (SecureRandom.random_number * 180 - 90).round(6),   # Generates a latitude between -90 and 90
    longitude: (SecureRandom.random_number * 360 - 180).round(6)   # Generates a longitude between -180 and 180
  }
end
...
puts "Connected to MQTT broker at #{BROKER_HOST}"

# Send message every second with fake GPS location
# loop do
#   fake_gps = generate_fake_gps
#   message = fake_gps.to_json
#
#   client.publish(TOPIC, message)
#   puts "Sent GPS data: #{message}"
#
