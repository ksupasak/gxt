...
require_relative '../lib/miot'
  ...
  set :endpoint, 'http://ppkintra.go.th'
...
# service to get patient data
  ...
  if ht = login
    ...
    http = ht[:http]
      ...
      uri = URI("#{settings.endpoint}/ppkservices/api/Vital/Brokerbmi")
      ...
      senses['bp_sys']=['1', 'BPsystolic','mmHg']
      senses['bp_dia']=['2', 'BPdiastolic','mmHg']
      # senses['bp_mean']=['3', 'MBP','mmHg']
      senses['pr']=['4', 'Pulse','bpm']
      senses['temp']=['5', 'Temperature','C']
      senses['weight']=['6', 'Weight','kg']
      senses['height']=['7', 'Height','cm']
      senses['bmi']=['8', 'Bmi','kg/m2']
      senses['spo2']=['9', 'O2sat','%']
      senses['rr']=['10', 'Respiration','bpm']
      ...
      data['Dodate'] = now.strftime("%Y%m%d")
      data['Dotime'] = now.strftime("%H%M%S")
